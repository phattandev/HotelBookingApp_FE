...
const navItems = [
  { to: '/partner', label: 'Tổng quan', indent: false, exact: true },
  { to: '/partner/hotels', label: 'Danh mục khách sạn', indent: false },
    ...
    <div className="fixed inset-0 w-full h-full overflow-hidden flex bg-slate-50">
      ...
      <aside className="w-60 bg-slate-900 flex flex-col shrink-0">
            ...
            return (
              <Link
                ...
                className={`flex items-center py-2.5 rounded-lg text-sm font-medium transition-all ${item.indent ? 'pl-7 pr-3' : 'px-3'
                    ...
                    ? 'bg-sky-600/20 text-sky-300 border-l-2 border-sky-500 pl-[10px]'
        ...
        <div className="px-4 py-4 border-t border-slate-800 flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-sky-600/30 text-sky-300 flex items-center justify-center text-sm font-bold uppercase shrink-0">
        ...
        <header className="bg-white border-b border-slate-200 h-14 flex items-center px-6 justify-between shrink-0">
          <div className="flex items-center gap-2 text-sm text-slate-500">
            ...
            <span className="font-medium text-slate-700">
              {navItems.find(n => location.pathname.startsWith(n.to))?.label ?? 'Partner Panel'}
            </span>
          </div>
          <span className="text-[11px] bg-sky-100 text-sky-700 font-semibold px-2.5 py-1 rounded-full uppercase tracking-wide">
            Partner
          </span>
        </header>
